...
#import "DCWebViewJavascriptBridge_JS.h"

@implementation DCWebViewJavascriptBridgeBase {
...
- (void)flushMessageQueue:(NSString *)messageQueueString{
    ...
    for (WVJBMessage* message in messages) {
        ...
        if (responseId) {
            DCWVJBResponseCallback responseCallback = _responseCallbacks[responseId];
            responseCallback(message[@"responseData"]);
            [self.responseCallbacks removeObjectForKey:responseId];
        } else {
            DCWVJBResponseCallback responseCallback = NULL;
            ...
            DCWVJBHandler handler = self.messageHandlers[message[@"handlerName"]];
...
- (void)injectJavascriptFile {
    NSString *js = DCWebViewJavascriptBridge_js();
...
- (BOOL)isSchemeMatch:(NSURL*)url {
    ...
    return [scheme isEqualToString:kDCNewProtocolScheme] || [scheme isEqualToString:kDCOldProtocolScheme];
}

- (BOOL)isQueueMessageURL:(NSURL*)url {
    NSString* host = url.host.lowercaseString;
    return [self isSchemeMatch:url] && [host isEqualToString:kDCQueueHasMessage];
}

- (BOOL)isBridgeLoadedURL:(NSURL*)url {
    NSString* host = url.host.lowercaseString;
    return [self isSchemeMatch:url] && [host isEqualToString:kDCBridgeLoaded];
}
